	...
	integer infreq(2),indexa(750),ir(1)                                        0590 
	real name(750),nametmp,mu,lambda,wsa
      double precision X(750),Y(750),W(750),XO(2),YO(2),sR(2),
     c       wmin,temp,wtemp(750),denom,wbar(10000),wbar2(10000)

	OPEN(8,FILE='C:\Users\pxs101220\Documents\AAG\weber-out100.txt') 
	OPEN(9,FILE='C:\Users\pxs101220\Documents\AAG\weber-out-wt10.txt') 
   ...
   40	continue
	...
	w(i)=real(ir(1)) + 1.0
	wbar(k)=wbar(k)+w(i)
	wbar2(k)=wbar2(k)+w(i)**2
 
c	w(i)=1.0

   34 CONTINUE 

	wbar(k)=wbar(k)/real(n) 
	wbar2(k)=wbar2(k)/real(n-1) - real(n)*(wbar(k)**2)/real(n-1)
	write(9,*) k,wbar(k),wbar2(k)
c	write(6,*) denom,wbar(k),wbar2(k)
c	pause
...
c     WRITE(7,200) N                                                       1010 
  200 FORMAT (' NUMBER OF DESTINATIONS:',I4/                               1020 
     ...
     * 'CO-ORDINATES',12X,'WEIGHT   ALLOCATION'/1X,61('=')//)              1050 
      DO 35 I=1,N                                                          1060 
c      WRITE(6,201) I,X(I),Y(I),W(I),name(i)                                1070 
...
c 	w(ii)= 1.0d-10
  ...
  998	write(8,5000) k,xo(1),yo(1),xo(2),yo(2),rmin,(infreq(j),j=1,2),
     c              wbar(k),wbar2(k)

  999 STOP   
 5000 format(1x,i5,1x,2(2f10.6,2x),d10.5,2x,2i5,2f10.5)
      END   
c
c
c
